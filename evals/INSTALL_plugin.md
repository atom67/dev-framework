...
2. Otherwise run in the terminal:
   ...
   (a public user would write `hermes plugins install atom67/dev-framework`). Plugins load when a session starts,
   so you cannot use the new tools in this session. Reply with exactly one line —
   `PLUGIN INSTALLED — start a new session and send this prompt again` — and stop. Do nothing else.
