...
class AgentTests(WorkspaceTest):
    ...
    def test_brief_one_block_remote_state_and_checklist(self):
        ...
        _, out = self.nav("health", "db,tx")
        self.assertTrue(out.startswith("| SCOPE-1 |") and "| DB-1 |" in out and "| TX-1 |" in out, out[:200])
        self.assertNotIn("| UI-1 |", out, "only the areas the task touches")
