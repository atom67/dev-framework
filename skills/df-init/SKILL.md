---
name: df-init
description: Install the DEV Framework into a repository (documents + verification gates), then fill its project facts. Use when the operator says "install DEV Framework", "set up the framework", "init the project", "поставь фреймворк", or promotes a kind (explore → tool → product).
argument-hint: <target-repo-path> [project name] [scale, e.g. "100 users"]
allowed-tools: Bash, Read, Write, Edit, Glob, Grep
---

`<PLUGIN>` below is this plugin's root: two directories above this skill's own directory (`${CLAUDE_PLUGIN_ROOT}` in Claude Code).

Install the DEV Framework into the repository named in the operator's arguments (`$ARGUMENTS`) (default: the current project).
...
1. Ask the operator, **one question per message**, only what was not given (do not announce a total: how many
   ...
   4. only for a product: **testing depth** — `lean` (recommended below 10,000 users: one scenario test per
      user promise) or `advanced` (recommended from tens of thousands of users, or with a production
      environment and a high cost of error). Recommend from the scale; the operator decides. Explorations and
      tools are lean without asking;
   5. **devlog**: keep a verbatim log of every finished dialogue? Default no; in a public repository it stays local
...
2. Preview, then install (`--scale` only for a product):
   ...
   ```bash
   python "<PLUGIN>/scripts/install.py" --target <TARGET> --name "<NAME>" --kind <KIND> [--scale "<SCALE>"] --profile generic [--testing lean|advanced] --devlog|--no-devlog --dry-run
   python "<PLUGIN>/scripts/install.py" --target <TARGET> --name "<NAME>" --kind <KIND> [--scale "<SCALE>"] --profile generic [--testing lean|advanced] --devlog|--no-devlog
   ```
...
5. the `df-check` skill with `<TARGET> hostcheck` — host rules that fight the framework. Show the operator the list
   **once**, one decision per line; their decision, never yours.
6. the `df-check` skill with `<TARGET> doctor` → fix what it lists → the `df-check` skill with `<TARGET> finish` once tests exist.
