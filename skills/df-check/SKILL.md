---
name: df-check
description: Run a DEV Framework gate — doctor, finish, commit-check, secrets or hostcheck. Use when the operator asks to check, verify or gate the work ("run doctor", "finish", "is it ready", "проверь", "прогони гейт") or to find host rules that fight the framework.
argument-hint: "[target] doctor|finish|commit-check|secrets|hostcheck"
...
Run the gate named in the operator's arguments (`$ARGUMENTS`) (default: `doctor` in the current project) and report its verdict:

```bash
python <TARGET>/.devframework/check.py doctor          # structure + configuration; READY or the setup items
python <TARGET>/.devframework/check.py finish          # + secret heuristic + build + tests (> 0 run, none failed) + checks
python <TARGET>/.devframework/check.py commit-check    # finish + staged secret scan, before an authorized commit
